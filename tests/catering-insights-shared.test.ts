...
import { formatMonthLabel } from "@/lib/i18n/format";
...
describe("formatMonthLabel (the calendar's month header)", () => {
  it("renders month + year in the app language, never shifting the month", () => {
    expect(formatMonthLabel("2026-09", "en")).toBe("September 2026");
    expect(formatMonthLabel("2026-01", "en")).toBe("January 2026");
    expect(formatMonthLabel("2026-12", "es")).toBe("diciembre de 2026");
  });
  it("returns garbage verbatim rather than rendering an invalid date", () => {
    expect(formatMonthLabel("nope", "en")).toBe("nope");
    expect(formatMonthLabel("2026-13", "en")).toBe("2026-13");
  });
});
