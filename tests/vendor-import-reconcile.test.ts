...
describe("vendor import reconciliation", () => {
  it("normalizes Butter case quote to its each purchase root", () => { expect(priceAtRoot(row("36/1 LB", { price_cents: 8030 }), root(16), "per_each")).toBe(2.23); expect(packComparison(row("36/1 LB"), root(16), "per_each").same).toBe(true); });
  it("normalizes weight quote at the proven root", () => expect(priceAtRoot(row("8 LB", { price_per_lb_cents: 629 }), root(128), "per_lb")).toBe(50.32));
  it("normalizes volume without assuming mass", () => expect(priceAtRoot(row("4/1 GA", { price_cents: 7399 }), root(128, "volume", "fl oz"), "per_each")).toBe(18.50));
