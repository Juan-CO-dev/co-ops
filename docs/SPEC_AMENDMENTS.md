...
## C.55 — Cross-user mark-not-done authority (the C.28 extension C.28 never decided)

**Date added:** 2026-06-04
**Status:** Authored (locked); build in progress this session
**Spec sections:** §2.5 (immutable completions), §4.3 (`checklist_completions`), §5.2 (RLS on `checklist_completions`); extends C.28 §3–§5
**Extends:** C.28 (revocation + accountability tagging two-window architecture)
**Triggered by:** C.41 role-renumber (migration 0058) smoke, 2026-06-04 — verifying the renumbered KH+ gate surfaced that **no actor, at any level (including CGS/owner), can reopen a false tick on someone else's completed checklist row.** C.28 gave KH+ a *tag* affordance on others' rows (re-attribute who did the work) but the *reopen* (soft-revoke) path stayed self-only (`revokeWithReason` enforces `completion.completed_by === actor.userId`). A KH watching an employee tick "Lock back door" when the door is plainly unlocked has no in-system way to reopen the item — only the employee who tapped it can. That is the operational gap.

**What spec says:** C.28 §3 established "KH+ peer correction (any time after 60s)" but scoped it to **tag only** — annotate `actual_completer_id`, never touch the completion's live/revoked state. C.28 §1–§2 scoped *revocation* (silent self-undo + structured self-after-60s) to **self only**. Cross-user mark-not-done (one actor reopening another actor's completion) was never decided — it falls in the seam between "KH+ can tag others" and "only self can revoke." C.28 §5's two-truths model makes it *possible* (revoke is soft — sets `revoked_at`/`revoked_by`, preserves the append-only `completed_by`) but C.28 never authorized it.

**What built reality is (and intended for this build):** a NEW capability — **cross-user mark-not-done** — that lets a sufficiently-senior actor reopen another actor's completed row. Locked rules:

1. **Floor = KH+ (`actor.level >= 4`).** Below KH stays self-only, unchanged. An employee (level 3) sees nothing new; their correction surface is exactly C.28's self-only path.

2. **Bound = `actor.level >= target_completer.current_level` (AT-OR-BELOW, peers/lateral INCLUDED).** A KH can reopen a peer KH's tick (`>=`, not strict `>`). A KH cannot reopen a GM's tick (`4 >= 6` is false → 403). Level 7+ (owner/CGS) clears everyone via the same predicate. This is **deliberately different from `lib/roles.ts` `canActOn`** (strict-greater, admin-acts-on-junior semantics) — cross-user mark-not-done is lateral-inclusive, so it gets its own predicate and its own error class, NOT a `canActOn` reuse.

3. **Completer level is the completer's CURRENT level**, resolved server-side at action time via the C.28 `tagActualCompleter` service-role pattern (`users.role` → `getRoleLevel`), NOT a level frozen at completion time. Accepted trade-off: consistency-with-tag (which also uses current level) beats frozen-level. The promotion edge is registered and accepted — if a completer is promoted above the actor after completing, the actor can no longer reopen that completion; this is rare and the senior actor or the now-senior completer can act.

4. **After-60s only.** During the C.28 60s quick-window (`QUICK_WINDOW_MS = 60_000`), cross-user mark-not-done is blocked — the window belongs to the completer's silent self-undo. A cross-user attempt inside it returns the `use_quick_revoke` signal (the completer should self-undo; the senior actor waits out the window).

5. **Note required, unconditionally.** Unlike C.28's self-revoke (where `error_tap` needs no note and `not_actually_done` needs none), cross-user mark-not-done always requires a free-form note — the actor is acting on someone else's record, so the reason must be captured. Reason category is fixed (no chip choice); the note is the captured intent.

6. **Distinct audit action: `checklist_completion.revoke_by_authority`** (NOT C.28's `checklist_completion.revoke`). A cross-user reopen is a categorically different event from a self-revoke and gets its own action code for an honest forensic trail. Added to `lib/destructive-actions.ts`; `isDestructive()` auto-derives `destructive=true` via registry membership. Metadata captures `instance_id`, `template_item_id`, `prior_completed_by`, `completer_level`, `actor_level`, `note`, `elapsed_ms`.

7. **Everything else from C.28 unchanged.** Tag (re-attribute) stays KH+ OR self. Self-undo (silent 60s), self-revoke-with-reason (post-60s), and the picker are all untouched. Cross-user mark-not-done is *additive* — a new path alongside C.28's, not a modification of any existing one.

8. **Reopened items re-complete via the normal flow.** A reopened row returns to incomplete and is re-tickable by whoever the normal completion gate allows. No special re-completion handling.

**Three-layer enforcement (CRITICAL — UI gate is cosmetic-only):**
- **Lib layer (the real enforcement):** new `markNotDoneByAuthority` in `lib/checklists.ts` performs the floor check (`actor.level < 4` → role violation), the quick-window check, the server-side completer-level resolution, the `actor.level >= completer.level` bound (else a new `ChecklistRevokeHierarchyViolationError`), and the note-required check, BEFORE the soft-revoke UPDATE.
- **RLS layer:** unchanged — `checklist_completions` user writes are denied (`_no_user_update`); the lib writes via service-role, exactly as C.28's tag/revoke do. Effective layers here = lib + UI.
- **UI layer (cosmetic only):** the new mark-not-done affordance shows when `isCompleted && !isActorCompletedBy && actorLevel >= 4 && actorLevel >= completerLevel && interactable && callback present`. **The completer level is SERVER-RESOLVED and passed down as a row prop — the client NEVER does role lookups.** A stale/wrong client-side level can only wrongly-SHOW a button that the server then 403s; it can never grant authority. The right-slot single-button becomes a correction menu when both tag + mark-not-done are live for a row.

**Why:** C.28's append-only model correctly preserves "what happened in the system" (the tap is never erased) while soft-revoke lets the operational state be corrected. C.28 authorized self-correction and KH+ re-attribution but left the cross-user *reopen* undecided — so a manager who directly observes a false completion on a junior's row has no in-system correction path, which is exactly the operational authority a KH+ role is supposed to carry on the floor. The `>=` (lateral-inclusive) bound matches real shift dynamics: peer KHs cover for each other and need to correct each other's records; the strict-greater `canActOn` semantic (built for destructive admin actions on subordinates) is too narrow for floor correction. Note-required-unconditionally keeps the forensic trail honest when acting on someone else's work. The distinct `revoke_by_authority` action keeps cross-user reopens queryable as their own class for the future Reports Console data-quality signal (sibling to C.28 §6).

**Post-change correction-authority table:**

| Actor vs completer | Tag (re-attribute) | Mark-not-done (reopen) |
|---|---|---|
| Self, ≤60s | n/a (silent self-undo) | self silent undo (`error_tap`) |
| Self, >60s | self always | self always (`revokeWithReason`) |
| Below KH (lvl 3), other's row | no | **no** (unchanged) |
| KH+ (lvl ≥4), other's row, ≤60s | no (tag blocked in window) | **no** → `use_quick_revoke` |
| KH+ (lvl ≥4), other's row at-or-below, >60s | yes (KH+ OR self) | **YES (new) — `actor.level >= completer.level`** |
| KH+ (lvl ≥4), other's row ABOVE actor, >60s | yes (tag is KH+, no hierarchy bound) | **no → 403 `ChecklistRevokeHierarchyViolationError`** |

(Note the asymmetry: C.28 tag has NO hierarchy bound beyond the KH+ floor — a KH can tag a GM's row. Cross-user mark-not-done DOES have the `>=` bound. Reopening a completion is a stronger act than re-attributing it, so it carries the seniority constraint that tag does not.)

**v1.3 action:**
- New lib fn `markNotDoneByAuthority(authed, { completionId, actor, note, ipAddress, userAgent })` in `lib/checklists.ts` — floor + quick-window + server-resolved completer-level + `>=` bound + note-required, then soft-revoke UPDATE (reuse `revokeWithReason`'s write shape: `revoked_at`/`revoked_by` set; `revocation_reason` fixed; `revocation_note` = the required note), then audit `checklist_completion.revoke_by_authority`.
- New `ChecklistError` subclass `ChecklistRevokeHierarchyViolationError` (mirrors `ChecklistTagHierarchyViolationError`) → 403 mapping in `app/api/checklist/_helpers.ts`.
- New action `"checklist_completion.revoke_by_authority"` in `lib/destructive-actions.ts` (alongside `checklist_completion.revoke` + `.tag_actual_completer`).
- New thin route `app/api/checklist/completions/[id]/mark-not-done/route.ts` (mirrors `tag-actual-completer/route.ts`; body `{ note }`).
- `components/ChecklistItem.tsx`: new `completerLevel` prop (server-resolved), new `onMarkNotDone` callback prop, new `showMarkNotDone` gate, right-slot correction-menu when both tag + mark-not-done live, note-capture ExpandMode, new i18n keys, error-code union + `errorMessageFor` switch additions.
- Parent loader resolves each completion's completer-level server-side and passes it as the row prop (client never does role lookups).
- i18n: `lib/i18n/en.json` + `lib/i18n/es.json` `closing.*` (and shared) keys for the menu, modal, note prompt, and error.
- Smoke (preview URL, not production): (1) UI menu visually — KH sees mark-not-done on at-or-below rows, does NOT see it on above-them rows or in the 60s silent window, employee sees nothing; (2) server hierarchy gate via curl/probe — KH reopening a peer KH = allowed, KH reopening a GM = 403, employee = 403.
- Captured during the C.41 role-renumber smoke, 2026-06-04, when the renumbered KH+ gate verification surfaced that cross-user reopen exists for no one. Shape locked with Juan (six gated dimensions); Gate 5 carries the two build-time requirements (visual smoke for the menu; server-resolved completer-level with cosmetic-only UI gate).

---

## How to add an entry

1. Pick the next monotonic ID (`C.<n>` — current next: C.56).
