...
## LEAD RULINGS — Phase 3 close (CC, 2026-08-22; PR #290)

Phase 3 shipped the shadow cron, the ledger, the audit vocabulary, migration `0183` (authored, 🔒 M2 unapplied) and the one par-write authority. The lead review verified the architecture against the diff. Where a ruling below contradicts the task text above, **this section wins** — the task text is not corrected in place, so read the two together.

**CONFIRMED IN THE DIFF, and named here so a later phase does not "fix" them back:**
- **The cron's depletion gate is SUCCESS-ONLY, not `watermark === businessDate`.** The builder's own pre-PR pass found the plan's Task-3.7 spelling wrong in both directions, and a false skip WRITES (`recordParRunSkipped` replaces the day's ledger rows), so it was data loss rather than a no-op: a **zero-consumption day** materializes successfully with zero rows and leaves the watermark on yesterday, and a **`?date=` backfill** — which that route supports — has a watermark LATER than its target, so the strict-equality path deleted the correct rows for the very date being backfilled. The gate is now the materialize loop's own result (`depletionRows` membership) **OR** `watermark >= businessDate`; the watermark is still read and is still the skip row's recorded evidence. **Task 3.7's `!==` is OVERRULED.**
- **The arbiter runs BEFORE the mutation.** `par_suggestion_actions`' unique index is what makes "one human verdict per generation" true, so the claim precedes the par write. Written the other way it guarded nothing — two taps both land their UPDATE and one then gets a 409 saying nothing happened — and pre-0182 it moved a par and *then* threw, leaving a changed par with no audit row. The stated trade-off (a consumed generation with an unmoved par) is the right failure: the reverse orphan is a kitchen change with no accountability record.
- **Both idempotence exclusions.** The hysteresis prior is the newest row **strictly before** the run date, and the budget counts `applied` always but `would_apply` only from earlier runs. Together these are what make a re-invoked nightly run reach the same verdict; they are unit-pinned in `tests/dynamic-pars-run.test.ts`, not merely commented.
- Also verified: the **D16 slot-creation carve-out** (the ladder does not short-circuit it, so the aggregate keeps its subject), the **skip path as a WRITE** (advisory-null rows, never stale-computed), the **`resolvePar` zero-diff reduction** pre- and post-apply, **`parWriteColumns`' structural closure of the machine lane** for `kind: "admin"`, and the admin upsert's **value-changed-only pin grain**.

**F4 — the pure core may not read the clock. Fixed.**
`parWriteColumns` carried `input.appliedAt ?? new Date().toISOString()` on the revert pin. The module's own header promises *"PURE: client-safe, zero I/O"*, and the house law is that **time enters a pure function as a parameter** — a `new Date()` fallback means the same input stops producing the same output and every test of the pin asserts against the clock. The silent alternative (`?? null`) is worse: it would **drop the pin** for a caller that forgot the stamp, on the one column whose entire job is to stand until a human clears it. **A `"revert"` with a null/undefined `appliedAt` now throws.** `writeParFromSuggestion` already always passes `nowIso`, so no server change; test-pinned both ways (throws without, pins to exactly the stamp with).

**F5 — the denominator's zero case, and the parity that decides it. Fixed.**
`classifyParReason` silences on `perOrderUnitOz == null`; `computeCoverage` rejects `<= 0`. A SKU whose pack data resolves to a **0-oz order unit** therefore slipped the ladder (0 is not null), reached coverage, came back null, and was miscaused as `thin_history` — an errand pointing at the demand window when the fault is the pack chain. That is the builder's own **V6** principle turned on itself. The walker already draws the line correctly at its use site (`perUnitOz != null && perUnitOz > 0`, `lib/ordering.ts` buildRow), so the fix is **parity with the shipped surface, not a new rule**: `normalizePerOrderUnitOz` at `DemandSku` construction, so every downstream consumer inherits it instead of each remembering to re-check.

**F6 — the velocity reset's grain is COARSER than per-SKU in v1. Accepted, and now stated in both places.**
`VelocityInput.recipeEditedAt` documented *"the most recent recipe edit touching this SKU or its product"*; `loadLatestRecipeEditAt` supplies the latest recipe edit **globally**, so any recipe edit anywhere resets velocity for every SKU for `VELOCITY_MIN_PERSISTENCE_DAYS`. **Accepted for v1** — the error direction is conservative (an over-reset can only withhold a nudge, never fabricate one), velocity is suggestion-lane-only, and per-SKU attribution needs flatten-aware audit parsing on the arc's own named drop-candidate. **But a pure core must not promise a grain its caller cannot deliver**: both doc blocks now say so, and the upgrade path (return a `Map<skuId, string>`; no signature change) is named at the loader.

**Standing, unchanged:** 🔒 **M2 is the lead's**, on Juan's word. The auto tier's live population is **one SKU** and `sku_count_events` is 0 — say so in every remaining phase PR so nobody reads shadow-quiet as broken.

---

## LEAD RULINGS — Phase 4 close (CC, 2026-08-25; PR #291)

Phase 4 shipped the walker's suggestion payload, the accept/dismiss/revert route, the shadow banner, the reason lane, the event advisory, the read-only machine value on the SKU admin, and 78 i18n keys across en + es. The lead review verified the architecture against the diff. Where a ruling below contradicts the task text above, **this section wins** — the task text is not corrected in place, so read the two together.

**CONFIRMED IN THE DIFF, and named here so a later phase does not "fix" them back:**
- **The walk-time re-selection reads BOTH day-classes, paired on `run_date`.** Task 4.1's "the latest row per (sku, day_class = walkInstant.dayClass)" is **OVERRULED by its own arithmetic**: `computeCoverage` sums PER COVERED DAY, and a Thursday walk's horizon reaches into Friday — so it needs the WEEKEND rate, which the engine persists only on the weekend row. Filtering the read to one day-class would have silently nulled every horizon that crosses the boundary, on the one surface the arc exists to light. Pairing the halves on `run_date` is the other half of the call: the engine writes both in one pass, so a half from a different night means a partially-written ledger, and blending two runs' terms is a second opinion. Honest null instead.
- **`shadowMode` is read from the ledger's own `mode` column**, not from `PAR_AUTO_APPLY_ENABLED`. Reading the constant from `lib/ordering.ts` would have been an import cycle — and the observed fact is the better answer anyway: the banner claims something about *this shop's last night*, and a build constant is a claim about the deployment. A location the engine has never run at renders no banner and one honest line, rather than a zeroed errand list that reads as "everything is fine".
- **The route posts an IDENTITY, never a value.** The par written is `suggestion.suggestedPar`, re-derived server-side at the request's own walk instant; a stale generation is a 409. There is no payload shape that moves a par to a number the system did not choose — pinned by a source test.
- **`loadAppliedAutoMove` guards the pin.** A revert refuses with `409 nothing_to_revert` unless an `applied` row really exists. Without it a revert in shadow would set the PIN — the one column whose job is to stand until a human clears it — on a slot the machine never moved, punishing a future auto-move for something that never happened.
- Also verified: the **#283 co-render exclusivity** enforced in the ONE row builder with the number winning; the **three-tier silence ordering** (errands · faults · not-a-fault last) with **no "other" bucket**; the **tier-ceiling rule** (a read path may not acquire an auto verdict it cannot see the guards for); and **exact en/es key parity, 78/78**.

**F7 — the location bind belongs IN the lib, not only in the route. Fixed.**
`writeParFromSuggestion` and `dismissSuggestion` floored the ROLE at `PAR_WRITE_MIN` and said nothing about WHICH shop. **Level 7 is not all-locations** — `lockLocationContext`'s grant starts at level 9 (`lib/locations.ts`) — so a GM is bound to their assignment list, and without the bind a level-7 manager at one shop could move the other shop's par with the role gate fully satisfied. The builder flagged it rather than silently fixing it, which was right; the ruling is that **"the lib is the authority" (AGENTS.md) outranks Task 4.4's silence**, and the house precedent is `loadOnHandDerived` and every other location-scoped reader in `lib/counts.ts` doing exactly this bind inside the lib. Today the sole caller is the route, which binds — so nothing was reachable — but the Phase-5 sim harness, a fleet route, or a future admin affordance would each have had to remember. **The bind now runs inside both functions, before the arbiter claim and before any I/O** (a bind failure must not consume a suggestion generation), throwing `DynamicParsError(404, "not_found")`. The route's bind STAYS as the outer layer: defence in depth, not redundancy. **The `machine` kind is exempt by design** — it carries no actor and the nightly cron iterates locations on purpose.
