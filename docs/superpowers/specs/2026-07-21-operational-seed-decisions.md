...
**✅ Stage 3d Slicing + Veg DONE** (`scripts/seed/03d-recipes-portioned.ts`, commit `4261c0d`): 20 single-input portioned recipes (9 Slicing→BH deli/PFG, 11 Veg→PFG produce + BH peppers/pickles); `portioned=true` inputs, `batch_yield=1` Q6 estimate. Mortadella auto-placeholder.

### ✅ STAGE 3 COMPLETE — recipe-driven item wiring DONE (2026-07-21)
**41 production recipes wiring ALL 41 real prep items** to live composition on the canonical graph (SKU → recipe_input → recipe → recipe_output → item), across Sauces(9)/Cooks(6)/Sides(6)/Slicing(9)/Veg(11). The only 4 active items without a recipe = the AM-prep checklist yes/no prompts ("Meatball mix - ready?", "Cook Bacon?"…), correctly left. **Cost + readiness + prep-consumption now flow for the whole prep bench.** Open placeholder SKUs to fill (6): Worcestershire, Beef Base, White Wine, Vanilla Bean Paste, Dried Chives, Mortadella. Approximate recipes to refine: Ranch, Horsey Mayo, Vin, Chicken Cutlet, Antipasto Pasta. Deferred (Stage 6): the Meatballs recipe (needs a new Meatballs item), menu builds (Fun-guy), catering/seasonal recipes (Coleslaw/Strata/Breadpudding/etc.).

**⏳ NEXT = Stage 6 (catering menu — `menu_items` + catering pricing; the highest-leverage stage, lights up the dormant catering moat + funnel ⑤) — then Stage 7 (checklists reconcile). Ground the live `menu_items`/catering tables before authoring.**

### ✅ Stage 6 — Catering menu seed COMPLETE (2026-07-21)
Spec `docs/superpowers/specs/2026-07-21-stage6-catering-menu-seed-design.md`, plan `.../plans/2026-07-21-stage6-catering-menu-seed.md`. Scraped both Toast pages → `docs/seed/source/toast-menu-{regular,catering}.md`. Engine extended (`lib-recipe-seed.ts`: consumer `output_menu_item_id` + `createItemIfMissing`). Scripts (all on prod, committed): **6a** `06a-deferred-subitems.ts` (birthed Meatballs/Meatball Spice Mix/Green Goddess/Caesar Dressing) · **6b1** `06b1-menu-subs.ts` (24 sub menu_items) · **6b2** `06b2-sub-builds.ts` (24 consumer builds, 104 sub-item inputs) · **6c** `06c-sides-and-resale.ts` (6 items→sold_directly + 38 resale menu_items) · **6d** `06d-catering-packages.ts` (16 packages ×2 loc + 108 slot options). **Prod totals: 62 menu_items (24 subs + 38 resale), 24 portionable subs w/ costed builds, 6 sold_directly items, 16 catering_packages, 108 slot options.** Catering prices derive once Juan authors `catering_rate_rules` in the W1a UI. Open placeholders to fill: Sub Roll, Utz Ripples, Lemon Oil, Mixed Herbs (+ Stage-3 six). Deferred builds: Garlic Bread / MeatBall Parm / Stuffed Peppers / Bacon Caesar Pasta Salad (priced resale menu_items, composed-build later). The dormant catering moat (W1a/W4a/W4b/W4c) + funnel ⑤ now have real menu data.

### ✅ Stage 7 — Checklists + close-out COMPLETE (2026-07-21)
**Checklists were ALREADY fully seeded in earlier phases** (Standard Closing v2 61 items, Opening v1 80, AM Prep v1 40, Mid-day Prep v1 14/15 — all active, both locations; Closing v1 inactive/superseded). No checklist source in `docs/seed/`; the closing checklist from the distillation was folded into Standard Closing back in Phase 3. So "seed the checklists" was a no-op (reconciliation confirmed coverage; duplicating would violate the plan).
**Fold-in (Juan): smallwares supplies** (`scripts/seed/07-smallwares-skus.ts`, committed): 26 office/cleaning/misc supply SKUs (global, inventory_only; Frooties/Fruity Pebbles = food, inventory_only=false); 4 Trimark overlaps skipped by idempotency.
**⭐ Final wiring audit + cleanup (Juan: "make sure everything is wired as it should be"):** deactivated 4 pre-seed LEGACY test recipes that duplicated/dangled (`Hot Peppers` [dup + input to deactivated Baldor SKU], `AntiPasta2` [dup of Antipasto Pasta], `AntiPasta` + `Meatball Mix` [orphan, no output]) — via `execute_sql` UPDATE + 4 `recipe.deactivate` audit rows. Re-audit: **0 broken edges, 0 orphan/duplicate recipes, 45 production recipes (1 per prep item), 24 consumer recipes (1 per sub).** Full chain traced end-to-end (Crunchy Boi menu_item → prep items → SKUs @ vendor).

## ⭐⭐⭐ THE OPERATIONAL SEED IS COMPLETE (2026-07-21)
All stages on prod, verified clean. Final state: **17 vendors** (fully attributed) · **163 active SKUs** (62 inventory_only, 10 placeholders to fill) · **49 items** (6 sold_directly) · **45 production + 24 consumer recipes** · **62 menu_items** (24 portionable subs w/ costed builds) · **16 catering_packages + 108 slot options** (both locations). Checklists pre-existing + verified. The dormant catering moat (W1a/W4a/W4b/W4c), FR-a/FR-b, and funnel ⑤ now have real data behind them. **Open follow-ups (non-blocking):** Juan authors `catering_rate_rules` in the W1a UI (catering prices derive then); fill 10 placeholder SKUs (Sub Roll, Utz Ripples, Lemon Oil, Mixed Herbs, Worcestershire, Beef Base, White Wine, Vanilla Bean Paste, Dried Chives, Mortadella); refine 5 approximate recipes (Ranch/Horsey/Vin/Chicken Cutlet/Antipasto Pasta) + author deferred composed-side builds (Garlic Bread/MeatBall Parm/Stuffed Peppers/Bacon Caesar Pasta Salad); P Street price/par overrides; branch `claude/operational-seed` has NO PR yet — Juan's call to merge.
