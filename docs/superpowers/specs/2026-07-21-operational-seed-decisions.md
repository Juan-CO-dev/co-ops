...
**⏳ NEXT = seed Boar's Head SKUs** (from `docs/seed/source/boars-head-order-guide-vA.md` — deli/pepper/beverage, first dual weekday/weekend pars, reconcile the Stage-2 Baldor deli stand-in placeholders + the Pickle Chips/Ever Roast Chicken recipe placeholders) **→ then Stage 3d/3e Slicing + Veg** (portioned = single-input recipes: raw BH/PFG SKU → item).
