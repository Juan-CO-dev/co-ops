...
### ⏳ Stage 3 — Global item roster (`items`) + wire item↔SKU — NEXT
Seed the portioned/prepped **`items`** roster (the union of goods across guides; `items.location_id=NULL` + `is_default` → propagates per Q1) and connect items → their component SKUs via `item_components`. The 15 raw placeholders may fold into this (some are really items, not SKUs). Recipe sub-builds (aiolis/dressings/meatballs → the 38 recipes) are **Stage 5**; catering menu is **Stage 6**. Ground the live `items` table + existing `item_components` (Hot Peppers already links) before authoring.
