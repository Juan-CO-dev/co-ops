...
**DRAFT for CC review · 2026-09-18 · wave 1 groundwork only.** This document proposes the write design; this branch contains only offline normalization, comparison, tests, and reports. No import writer, migration, network operation, or production read was performed.
...
Two corrections to earlier assumptions need CC's review: V3-A's introductory “PFG MOXē” wording does not describe these CustomerFirst files (MOXē belongs to the forthcoming US Foods export). Its §11 egg-repeat explanation is contradicted by distinct shell/cooked export products and seed 37's two unresolved lines. Preserve the uniqueness rule, reconsider the identity explanation; do not silently edit the locked prior spec.

## 2. Adapter registry and observation contract

The registry chooses **vendor folder AND header signature**, not filename alone. An unknown vendor/header fails loudly without publishing partial outputs. PFG CustomerFirst v1 is implemented. A US Foods MOXē adapter will be added only after real files establish its headers, unit semantics, account fields and footer. It produces the same row schema; it is not routed through the PFG parser.

Receipt photos are a later adapter family for Baldor, Boar's Head, Thompson Delivers/Utz, Trimark, Cardinal, Whisked and Berger. Preserve original photo hash and page/region reference. OCR extracts a proposed observation; a person verifies vendor/account, printed item number, quantity/UOM, date, price basis and pack. Missing item numbers produce unresolved review records, never invented vendor identifiers. A receipt transaction is not a CustomerFirst last-purchase summary; invoice/line identity deduplicates actual transactions. No image/OCR work is built here.
...
Before a future writer ships: tests for replay-after-commit, interrupted transport, two competing guide saves, foreign section/line/SKU, inactive retained versus new placement, case-insensitive sections, duplicate appends, changed before-state, no-op tokens, account scope, whole-chain pack versions, unsupported price basis and frozen confirmed PO keys. Exercise the coordinator in sim only; verify rollback on a forced late failure and service-role-only grants. CC reviews the plan, schema and code; ordinary PR CI/build applies then. Juan retains production merge/application authority. US Foods and receipt adapters each need real fixtures before activation.

## 9. Open questions for Juan — floor answers only

1. Which shell eggs are stocked now: 517879 large (30 dozen) or 517842 medium (15 dozen), and are 439686 cooked eggs also ordered separately?
2. Is “Dried Chives” a reminder to use the same fresh chives already listed in Produce, or a separate stocked product?
3. Is Izzy MAIN the list staff currently opens to place PFG orders, with the laminate still governing the reading sequence?
4. Does Capitol Hill use a separate PFG account, and are its packs/prices and laminated sequence the same as P Street's?
5. Are the alternate parmesan, garlic, oregano, basil, onion and mozzarella lines backups, or have any permanently replaced the printed laminate item?
6. Is the approximately double-priced basil now intentional, and does the “White” onion line actually mean yellow onions?
7. Are the two vinegars, mint, strawberries and multifold towels regular stocked purchases, or occasional items that should stay off the standing guide?
8. For the pork description “496 REPLACE 416637,” is 474569 the item staff actually keys today, with the older number retired?
