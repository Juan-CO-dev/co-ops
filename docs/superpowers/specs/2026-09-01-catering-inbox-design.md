...
Toast has no submit/accept split: an order is placed and paid in one step. So a Toast catering order enters the pipeline **already `confirmed`**, keyed on `external_ref = Toast order guid`, `lead_source='toast_catering'`, event date = the order's `promisedDate` when scheduled else the business date, headcount null (Toast does not carry it), items in notes, assigned to the catering manager. A voided order → `lost` while the lead is pre-fulfilment (inquiry / quote_sent / confirmed). **A void that lands after the lead is `out` never moves it (Juan, 2026-09-04: lost would hide what happened) — the lead keeps its stage, gets a loud note with Toast's void time, and the scan ledger marks it `voided_after_out_needs_review` for a human to audit (comp / refund / mistake).**
