...
Suites: `runner`, `isolation`, `personas`, `fixtures` (parent-owned Node lifecycle contract), `opening` (G1-A Node contracts + browser journeys). Fixtures: `warm-history`, `cold-empty`, `incomplete-pack`, `over-1000`, `two-shop-divergent`. Unknown flags/names return 2. `--repeat N` repeats every test without overwriting earlier evidence. All four Chromium projects run serially: phone/tablet × en/es, fresh browser context for each test. Project locale is a tag; it does not change a persona's saved app preference. Persona login checks the actual seeded language.
