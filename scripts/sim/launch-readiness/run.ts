...
export const SUITES = ["runner", "isolation", "personas"] as const;
export const FIXTURES = ["warm-history", "cold-empty", "incomplete-pack", "over-1000", "two-shop-divergent"] as const;
export function parseArgs(args: string[]) {
  let suite = "runner", fixture = "cold-empty", dev = false, noRestore = false, repeat = 1;
  ...
  return { suite, fixture, dev, noRestore, repeat };
}
export function restoreFixture(): never { throw new Error("F2 required"); }
...
export async function main(args = process.argv.slice(2)) {
  ...
  try { options = parseArgs(args); } catch { const e = new Evidence(runId, "invalid"); e.manifest.reason = "usage: --suite runner|isolation|personas --fixture <known-id> [--dev] [--no-restore] [--repeat N]"; e.finalize([]); return 2; }
  ...
  try {
    ...
    stage = "fixture"; if (!options.noRestore) restoreFixture();
    stage = "server";
    ...
    evidence.manifest.status = code === 0 && !interrupted ? "pass" : "fail"; evidence.manifest.reason = code === 0 ? "contracts passed; F2 restore and F5 production build unverified" : "Playwright failed";
  } catch (error) {
    evidence.manifest.status = stage === "playwright" || stage === "identity" ? "fail" : "blocked";
    evidence.manifest.reason = stage === "fixture" ? "F2 required" : `${stage} ${interrupted ? "interrupted" : "failed or blocked"}`;
