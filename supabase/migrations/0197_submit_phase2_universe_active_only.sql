...
-- 2026-06-17. The 09-01 audit named this class: a law obeyed in one sibling and missed in
-- the next (BC-009 inactive rows read as live · BC-025 asymmetry).
--
-- THE FIX. Same body as 0066 (finalize provenance) + `AND cti.active = TRUE` in BOTH
-- places the universe is enumerated: the completeness count and the per-item recompute
-- loop. Deactivated items are template history, never a prep obligation. Additive only.
--
-- Verification (sim, LRA opening contract): before — 422 phase2_incomplete after all 36
-- active items saved; after — 200 and status phase2_complete. Source-pinned by
-- tests/opening-phase2-universe-active.test.ts.
