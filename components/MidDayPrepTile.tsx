...
import { MAX_MID_DAY_PREP_PER_DAY, type MidDayPrepDashboardState } from "@/lib/prep";
    ...
    <div className="rounded-xl border-2 border-co-border bg-co-surface p-4">
      ...
      {!state.hasTemplate ? (
        ...
        <>
          {state.instances.length === 0 ? (
            ...
            <ul className="mt-2 flex flex-col gap-1.5">
              {state.instances.map((inst) => {
                const provenance =
                  inst.status === "phase2_complete" && inst.confirmedAt
                    ? serverT(language, "dashboard.mid_day_prep.finalized_by", {
                        time: formatTime(inst.confirmedAt, language),
                        name: inst.confirmedByName ?? "—",
                      })
                    : null;
                return (
          ...
          <div className="mt-3">
            {state.instances.length >= MAX_MID_DAY_PREP_PER_DAY ? (
              <p className="text-[11px] italic text-co-text-muted">
                {serverT(language, "dashboard.mid_day_prep.cap_reached", {
                  cap: MAX_MID_DAY_PREP_PER_DAY,
                })}
              </p>
            ) : (
